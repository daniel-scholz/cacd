...
from diffae.data.base import BaseMRIDataset
from diffae.data.dataframe import map_col_to_int


class IXIDataset(BaseMRIDataset):
    """IXI dataset loader: brain T1/T2/PD at 1mm MNI152, organised by scanner site."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if spatial_dims == 2:
            assert 0 <= slices_around_middle <= (192 // 2), (
        ...
        print("Number of subjects in dataset:", len(self.subjects_fps))
        super().__init__(
            data_dir,
        ...
        if target_prop == "age":
            ...
            self.scanner_int, self.target_names = map_col_to_int(
                self.labels[target_prop]
            )

        sample_img = self[0]["img"]
        print("Shape of data:", sample_img.shape)
        assert sample_img.dim() == self.spatial_dims + 1, (
    ...
    def _load_labels(self):
        ...
        scanners = {
            self.ixi_id_int(sub_id): self._id2scanner(sub_id)
            for sub_id in self.subject_ids
        }
        df_scanners = pd.DataFrame.from_dict(
            scanners, orient="index", columns=["scanner"]
        )
        ...
        self.subjects_fps = [
            sub_fp
            for sub_fp in self.subjects_fps
            if self._isin_labels(self._id_from_fp(sub_fp))
        ]
    ...
    def _load_subject_fps(self, data_dir: Path, spatial_dims: Literal[2, 3]):
        ...
        self.subjects_fps = [
            sub_fp for sub_fp in self.subjects_fps if "mask" not in sub_fp.stem
        ]
    ...
    def loader(self, fp, slice_offset):
        ...
        nii_img_slices = nii_img.slicer[
            ...,
            nii_img.shape[-1] // 2 + slice_offset : nii_img.shape[-1] // 2
            + slice_offset
            + 1,
        ]
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | list[Path]]:
        ...
        mask_fp = subject_fp.with_name(
            subject_fp.name.replace(
                f"{self.mri_sequences[0]}.", f"{self.mri_sequences[0]}_mask."
            )
        )
    ...
    def get_patient_attrs(self, index):
        patient_attrs = {
            ...
            "scanner": self.labels.loc[
                self.ixi_id_int(self.subject_ids[index]), "scanner"
            ],
        }
    ...
    def load_anat_label_map(self, subject_fp, slice_offset):
        anat_label_map_fp = subject_fp.with_name(
            subject_fp.name.replace(
                f"{self.mri_sequences[0]}.", f"{self.mri_sequences[0]}_seg."
            )
        )
        anat_label_map = self.loader(anat_label_map_fp, slice_offset)
        anat_label_map = self.crop(
            {"img": anat_label_map[None], "seg": anat_label_map[None] > 0}
        )["img"][0]
