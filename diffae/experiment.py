...
from diffae.data.split import split_datasets
...
class LitModel(L.LightningModule):
    ...
    def _encode_stochastic(
        ...
        out = sampler.ddim_reverse_sample_loop(model, x, model_kwargs={"cond": cond, "imgs": imgs})
    ...
    def setup(self, stage=None) -> None:
        ...
        train_datasets = self.conf.make_datasets(
            self.conf.data_paths, split="train"  # type: ignore
        )
        val_datasets = self.conf.make_datasets(self.conf.data_paths, split="val")  # type: ignore

        seed = self.conf.seed
        data_split_mode = self.conf.data_split_mode
        data_names = self.conf.data_names
        test_datasets = self.conf.make_datasets(self.conf.data_paths, split="test")

        # train_data, val_data, test_data = split_datasets(
        #     train_datasets, test_datasets, seed, data_split_mode, data_names
        # )
    ...
    def _verify_reproducibility(self):
        """
        Verify that the splits are the same as in the *_subjects.csv
        files to ensure reproducibility.

        """
        split_to_subset_dict = {
            "train": self.conf.fit_sites,
            "val": self.conf.fit_sites,
            "test": self.conf.test_sites,
        }
        all_splits_datasets = [self.train_data, self.val_data, self.test_data]
        for split, split_datasets_ in zip(("train", "val", "test"), all_splits_datasets):
            subjects_fn = f"{split}.txt"

            for data_name, split_dataset in zip(  # type: ignore
                self.conf.data_names, split_datasets_.datasets
            ):
                # get all subjects from current split
                # is torch subset dataset, i.e. has indices
                cur_indices = split_dataset.indices
                cur_dataset: IXIDataset = split_dataset.dataset  # type:ignore

                # apply list(set(...)) to remove duplicates
                # when using multiple sequences of the same subject
                cur_dataset_subjects = list(set([cur_dataset.subject_ids[i] for i in cur_indices]))
                splits_dir = Path("dataset") / data_name
                if split_to_subset_dict[split][data_name]:
                    saved_split_subjects = []
                    for subset in split_to_subset_dict[split][data_name]:
                        if split == "test" and subset not in self.conf.fit_sites[data_name]:
                            # use all subjects for test if subset is not used for training
                            subjects_fn = "all.txt"
                        else:
                            subjects_fn = f"{split}.txt"

                        subjects_fp = splits_dir / subset / subjects_fn

                        with open(subjects_fp, "r", encoding="utf-8") as f:
                            saved_split_subjects.extend([line.strip() for line in f.readlines()])
                        # check that all files currently in the dataset are
                        # also in the split saved to the txt file
                    assert all(
                        cur_dataset_subject in saved_split_subjects
                        for cur_dataset_subject in cur_dataset_subjects
                    ), f"split {split} does not match {data_name} subjects"
                else:
                    subjects_fp = splits_dir / subjects_fn
                    with open(subjects_fp, "r", encoding="utf-8") as f:
                        saved_split_subjects = f.readlines()
                        saved_split_subjects = [f_sub.strip() for f_sub in saved_split_subjects]
                        # check that all files currently in the dataset are
                        # also in the split saved to the txt file
                        assert all(
                            cur_dataset_subject in saved_split_subjects
                    ...
                    print(f"split {split} matches {data_name} subjects")

    def _verify_no_leakage(self):
        """
        Load all train, val, and test subjects from csv files and verify
        that there is no leakage between splits.
        """
        split_to_subset_dict = {
            "train": self.conf.fit_sites,
            "val": self.conf.fit_sites,
        ...
        for i_data, data_name in enumerate(self.conf.data_names):
            all_subjects = []
            splits_dir = Path("dataset") / data_name
            # get any dataset to check if it has fit_sites and test_sites
            ...
            for split in ("train", "val", "test"):
                if split_to_subset_dict[split][data_name]:
                    for subset in split_to_subset_dict[split][data_name]:
                        ...
                        subset_split_subjects_fp = splits_dir / subset / subjects_fn
                        with open(subset_split_subjects_fp, "r") as f:
                            subset_split_subjects = f.readlines()
                            subset_split_subjects = [s.strip() for s in subset_split_subjects]
                            split_subjects.setdefault(split, []).extend(subset_split_subjects)
                else:
                    subjects_fn = f"{split}.txt"
                    with open(splits_dir / subjects_fn, "r") as f:
                        subset_split_subjects = f.readlines()
                        subset_split_subjects = [s.strip() for s in subset_split_subjects]
                        split_subjects.setdefault(split, []).extend(subset_split_subjects)
            is_leakage = (
                any(s in split_subjects["train"] for s in split_subjects["val"])
                or any(s in split_subjects["train"] for s in split_subjects["test"])
                or any(s in split_subjects["val"] for s in split_subjects["test"])
            )

            if is_leakage:
                leaked_set = set(s for s in all_subjects if all_subjects.count(s) > 1)
                raise ValueError(f"leakage in {data_name},{leaked_set}")
            print(f"no leakage in {data_name}")

    def _append_split_sites(self, cur_dataset, splits_dir):
        """Append the fit_sites and test_sites to the splits_dir if availabe."""
        if hasattr(cur_dataset, "fit_sites") and hasattr(cur_dataset, "test_sites"):
            sites_str = "-".join(cur_dataset.fit_sites) + "_" + "-".join(cur_dataset.test_sites)
            splits_dir = splits_dir / sites_str
    ...
    def validation_step(self, batch, batch_idx) -> dict:
        ...
        for pred_imgs, pred_imgs_name, imgs in zip(pred_imgs_list, pred_img_names_list, imgs_list):
    ...
    def _compute_image_quality_metrics(
        ...
        pred_imgs_224 = F.interpolate(pred_imgs, size=224, mode="bilinear", align_corners=False)
        pred_imgs_01 = (pred_imgs + 1) / 2

        # todo: check preds, target
    ...
    def log_cond_weights(self):
        ...
        for name, module in conditoned_modules.items():
            if not hasattr(module, "cond_emb_layers"):
                continue
            weight_per_cond_entry = module.cond_emb_layers[1].weight.norm(dim=1).detach()
        ...
        for prefix in ["input", "middle", "output"]:
            weights = [
                v for k, v in cond_weight_dict.items() if k.startswith(f"cond_weight/{prefix}")
            ]
            if weights:
                cond_weight_dict[f"cond_weight/{prefix}"] = torch.stack(weights).mean()
                cond_weight_dict[f"cond_weight/{prefix}_std"] = torch.stack(weights).std()

        self.log_dict(cond_weight_dict, sync_dist=False)

    def shared_step(self, batch: dict[str, torch.Tensor], batch_idx, step_name: str) -> dict:
        """
        ...
        if (
            ...
            and ((self.global_step) % (self.conf.vis_every_steps * 10) == 0)  # frequent vis
        ...
        if self.is_first_accum(batch_idx):
            if self.conf.vis_every_steps > 0 and (
                ...
                if self.global_rank == 0:
                    paired_analysis(self.conf, self, None)
    ...
    def log_latents(self):
        ...
        if z_ids.size(1):
            ...
            if latents_fig_z_id is not None:
                ...
                self.logger.log_image("latents/z_id", [wandb.Image(plt_img)], step=self.global_step)
        if z_sems.size(1):
    ...
    def log_sample_with_model(
        ...
        with torch.no_grad():
            ...
            gen = self.eval_sampler.sample(model=model, noise=noise, cond=cond, x0=x0, imgs=imgs)
...
def train(conf: TrainConfig, fast_dev_run: bool = False):
    ...
    if not fast_dev_run:
        ...
        # taken from the tutorial, not sure if it's needed

        cur_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        conf.logdir = Path(conf.logdir) / "wandb" / f"run-{cur_time}-{conf.wandb_id}" / "files"
    ...
    def create_checkpoint(monitor: Optional[str], save_last: Optional[bool] = None, **kwargs):
        # define defaults
        save_top_k = 1
    ...
    trainer = L.Trainer(
        profiler=(
            ...
            "simple"
            if conf.name == "profile"
            else None
        ),
        # use cpu if on macos else "auto"
        accelerator=(
            "cpu" if torch.backends.mps.is_available() or conf.device == "cpu" else "auto"
        ),
