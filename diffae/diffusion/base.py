...
class GaussianDiffusionBeatGans(nn.Module):
    ...
    def __init__(self, conf: GaussianDiffusionBeatGansConfig):
        ...
        self.register_buffer(
            "alphas_cumprod", torch.cumprod(alphas, dim=0), persistent=False
        )
        self.register_buffer(
            "alphas_cumprod_prev",
            torch.cat(
        ...
        self.register_buffer(
            ...
            torch.log(
                torch.cat(
                    [self.posterior_variance[1][None], self.posterior_variance[1:]]
                )
            ),
            persistent=False,
        )
    ...
    def training_losses(
        ...
        **model_kwargs,
        ...
        terms["mse_loss"] = self.mse_loss(
            model_output.flatten(1), target.flatten(1)
        ).mean(dim=1)
        ...
        if isinstance(model.model, DiffAEIDModel):
            ...
            z_sem, z_id = torch.split(
                cond, [self.conf.z_sem_dim, self.conf.z_id_dim], dim=1
            )
            ...
            if l_content_loss > 0:
                ...
                labels_content = torch.arange(batch_size, device=z_id.device).repeat(
                    n_views
                )

                content_attract_mask = labels_content.unsqueeze(
                    0
                ) == labels_content.unsqueeze(1)
                match self.conf.double_gin_scanner_contrasting:
                    case "same_patient_negative":
                        ...
                        same_scanner_mask = labels_scanner.unsqueeze(
                            0
                        ) == labels_scanner.unsqueeze(1)
                ...
                terms["loss"] = (
                    terms["loss"] + l_content_loss * terms["content_loss"].mean()
                )
            ...
            if l_scanner_loss > 0:
                ...
                match self.conf.double_gin_scanner_contrasting:
                    case "same_patient_negative" | "same":
                        ...
                        labels_patient = torch.arange(
                            batch_size, device=z_id.device
                        ).repeat(n_views)
                        ...
                        for i_scanner, cur_scanner in enumerate(unique_scanners):
                            ...
                            if (
                                self.conf.double_gin_scanner_contrasting
                                == "same_patient_negative"
                            ):
                            ...
                            terms[f"scanner_{unique_scanners[i_scanner]}_loss"] = (
                                cur_scanner_loss
                            )
                ...
                terms["loss"] = (
                    terms["loss"] + l_scanner_loss * terms["scanner_loss"].mean()
                )
            ...
            if l_cross_correlation > 0:
                ...
                terms["loss"] = (
                    terms["loss"] + l_cross_correlation * cross_correlation_loss
                )
    ...
    def calc_cross_correlation_loss(self, z_sem, z_id):
        ...
        cross_correlation_matrix_samples = (
            z_sem_norm[..., None] @ z_id_norm[..., None, :]
        )
        ...
        cross_correlation_loss = torch.norm(
            torch.triu(cross_correlation_matrix), p="fro"
        )
    ...
    def p_mean_variance(
        ...
        model_forward = model.forward(
            x_t=x_t, t=self.rescale_timesteps(t), **model_kwargs
        )
        model_output = model_forward.pred
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        eps = eps - (1 - alpha_bar).sqrt() * cond_fn(
            x, self.rescale_timesteps(t), **model_kwargs
        )
    ...
    def ddim_sample_loop_progressive(
        ...
        indices = torch.arange(
            self.num_timesteps - 1 - T_offset, end=-1, step=-1, device=device
        )
        if progress:
            indices = tqdm(indices)
