...
class AnatomicalRegionsAugmentation(nn.Module):
    ...
    def sample_params(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        gamma_reg = torch.exp(
            torch.randn(batch_size, self.n_anat_regs, device=self.device) * self.gamma_std
        )

        # random scale sampled from N(+-1, 0.2)
        scale_reg = (
            torch.randn(batch_size, self.n_anat_regs, device=self.device) * self.scale_std
            + self.scale_mean
        )
        scale_reg *= (
            (torch.rand(batch_size, self.n_anat_regs, device=self.device) * 2 - 1).sign().float()
        )

        # random bias drawn from N(0, 0.2)
        bias_reg = torch.randn(batch_size, self.n_anat_regs, device=self.device) * self.bias_std
    ...
    def generate_augmented_image(
        ...
        img_mean = (
            imgs.flatten(start_dim=1).mean(dim=1).view(imgs.shape[0], *(1,) * (imgs.dim() - 1))
        )
    ...
    def anat_labels_to_oh(self, anat_seg: torch.Tensor) -> torch.Tensor:
        ...
        anat_segs_oh = (
            anat_seg[:, None] == anat_segs_unique[None, :, *(None,) * (anat_seg.dim() - 1)]
        ).float()
...
class LabelSymmetrizer(nn.Module):
    ...
    def __init__(self, device: torch.device):
        ...
        symm_anat_label_map = {k: self.strip_left_right(v) for k, v in self.anat_label_map.items()}
        symm_anat_label_map = {k: self.unify_ventricles(v) for k, v in symm_anat_label_map.items()}
    ...
    def forward(self, anat_segs: torch.Tensor) -> torch.Tensor:
        return self.label_to_unique_symm_label_array[anat_segs.flatten().tolist()].reshape(
            anat_segs.shape
        )
