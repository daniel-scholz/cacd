...
class RandBiasFieldCorruptionAugmentation(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        bias_field_transform = self.sample_rand_bias_field(device=image.device, dtype=image.dtype)
    ...
    def sample_n_transforms(self, n: int, device: torch.device, dtype: torch.dtype):
        return [self.sample_rand_bias_field(device=device, dtype=dtype) for _ in range(n)]
...
class _RandBiasField(nn.Module):
    ...
    def __init__(
        ...
        bias_field_small_spatial_shape = tuple([int(np.ceil(s * bias_scale)) for s in img_size])
