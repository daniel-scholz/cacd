...
class TrainConfig(BaseConfig):
    ...
    net_double_gin_scanner_contrasting: Literal[
        "all", "same", "same_patient_negative"
    ] = "same"
    ...
    def make_datasets(
        ...
        dataset_dict = {
            "ixi": lambda path: IXIDataset(
                ...
                augmentations=(
                    GeometricAugment(
                        self.img_size, self.dims, dict_mode=True, keys=["img", "seg"]
                    )
        ...
        return tuple(
            dataset_dict[data_name](path)
            for data_name, path in zip(self.data_names, paths)
        )
    ...
    def make_loader(
        ...
        return DataLoader(
            ...
            pin_memory=True,
            drop_last=drop_last,
            persistent_workers=(num_worker or self.num_workers) > 0,
        )
    ...
    def make_model_conf(self):
        ...
        print(
            f"{self.eval_every_steps=} * {self.accum_batches=} = {self.eval_every_batch=}"
        )
    ...
    def update_config_from_json(self):
        conf_dict = {}
        if (
            conf_json := Path(__file__).resolve().parent.parent
            / "conf"
            / f"{self.name}.json"
        ).exists():
            ...
            assert all(conf_key in self.__dict__ for conf_key in conf_dict.keys()), (
